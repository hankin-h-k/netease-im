...
func (c *ImClient) SendAttachMsg(from string, msgtype int, to string, attach string, opt *ImSendAttachMessageOption) error {
	param := map[string]string{"from": from, "msgtype": strconv.Itoa(msgtype), "to": to, "attach": attach}
